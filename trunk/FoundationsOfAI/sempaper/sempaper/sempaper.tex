...
\end{algorithmic}
\end{algorithm}
Temporal difference learning algorithms using a parameter $\lambda$ for eligibility traces are often abbreviated as TD($\lambda$). Figure \ref{backupDiagram} shows a backup diagram of TD($\lambda$). There are two special cases: If $\lambda=0$ then the scenario is equal to one-step TD methods (like discussed before). If $\lambda=1$, then the algorithm turns to a Monte-Carlo algorithm, because the value of a state depends on the whole set of successor states until the final state. But how to choose the parameter $\lambda$ appropriately? There is no clear answer to this question, but in tasks with long delayed rewards it makes sense to use quite a high value in order to accelerate convergence. Nevertheless the computational effort of eligibility traces is higher than using one-step prediction methods. 
\begin{figure}[htbp]
\center
\includegraphics[clip,keepaspectratio,scale=0.5]{pics/backupDiagram.png}
\caption{A backup diagram of TD($\lambda$) (taken from \cite{Sutton98})}
\label{backupDiagram}
\end{figure}
\subsection{Applications of reinforcement learning}
The idea of learning by interacting with the environment and receiving feedback is very intuitive and so there are also many practical applications of reinforcement learning. In order to give a brief idea, two interesting domains will be presented in this section. Some other interesting stuff can be found on the homepage of the annual reinforcement learning contest (\cite{learningContest}).
\paragraph{TD-Gammon}
Todo
\paragraph{Elevator dispatching}
Todo
%\begin{figure}[htbp]
%\label{elevatorImage}
%\center
%\includegraphics[clip,keepaspectratio,scale=0.5]{pics/elevator.png}
%\caption{An overview of the elevator domain}
%\end{figure}
...
The following table gives an overview of the results of the games played by the agents.\\
...
Since reinforcement learning has been a topic of interest in artificial intelligence research for quite a long time, there is lots of information to be found in books, papers or on the internet. The most fundamental one which is also available online is the book "Reinforcement Learning- An Introduction" (\cite{Sutton98}) written by Sutton and Barto. It explains the basics of reinforcement learning illustrated by intuitive examples. The thesis by Irene Markelic gives even a more intuitive understanding of the topic with less theoretical background. She applied the SARSA($\lambda$) algorithm to a robot soccer task and also explained the principles of reinforcement learning. Littman's paper (\cite{Littman94}) extends the single agent scenario to a multi-agent one, limiting the world to two agents. Nevertheless important multi-agent phenomena such as cooperation are also not considered there.
