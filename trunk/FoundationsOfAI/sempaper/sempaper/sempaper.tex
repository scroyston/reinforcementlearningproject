...
In contrast to other methods of machine learning, an agent only learns by receiving feedback from its environment and trying out which actions yield the highest return in a certain state. It does not seem astonishing at all that one of the roots of this method of learning can be found in psychology. An illustrative example is comparing this method to the way infants learn when making their first steps in an unknown environment. For example crying will often lead to the result that the parents get something to drink or to eat for the child, which can be regarded as a reward. So crying would be learned as a good action when trying to reach the goal of getting food.  Another analogy is the situation when learning to drive (also referred to in \cite{Markelic}). Neglecting the driving instructor who could turn this example into one for supervised learning, driving can also be learned by paying attention to the reactions of the environment (e.g. the car and other road users). One goal could be to get no bad reaction of the other drivers, so honking could be a bad signal from the environment and remind you to drive faster for example. More of those analogies can be found in \cite{Sutton98}.\\
...
In many environments the state and the action space are too huge to be explored completely. Due to this facts there have to be methods to reduce the dimension of those spaces without losing important information. This could be done by means of state-space abstraction.
...
Let $\mathcal{S}$ be a set of states and $\mathcal{A}$ be a set of actions an agent can choose in its environment.\\
A mapping $\pi:\mathcal{S} \rightarrow \mathcal{A} \ for\ all\ s \in \mathcal{S}$ is called a policy .
\end{definition}
...
\end{figure}
The algorithm in pseudocode is given in Algorithm \ref{policyIterationAlgorithm}.
\begin{algorithm}[tb]
...
\begin{center}
  ...
  \end{tikzpicture}
\caption{Initial state of the grid world with rewards for the actions}
...
\begin{center}
 \begin{tikzpicture}
            ...
            \draw [->] (f.west) -- (e.east);	
  \end{tikzpicture}
\caption{state after one value update}
\end{center}
\end{minipage}
\begin{minipage}[t]{0.475 \textwidth}
\begin{center}
  \begin{tikzpicture}
            ...
            \draw [->] (f.west) -- (e.east);	
  \end{tikzpicture}
\caption{state after all value updates}
\end{center}
\end{minipage}
\begin{minipage}[t]{0.475 \textwidth}
\begin{center}
  ...
  \end{tikzpicture}
\caption{state after policy change}
\end{center}
\end{minipage}
\begin{minipage}[t]{0.475 \textwidth}
\begin{center}
...
The curse of dimensionality already mentioned for single agent tasks becomes even worse when considering more agents. Now there is no longer only a state-action pair, considering the actions of one agent in agiven state, but the action combinations of all agents have to be regarded and stored in a table. This leads to a complexity that is exponential in the number of agents.\\
...
Another possibility to circumvent the coordination problem is learning models of the other agents, i.e. estimating the probability of an agent's action given a specified state.
\begin{figure}[tb]
\center
...
\end{enumerate}
\paragraph{Results}
An overview of the results can be found in Littman's paper (\cite{Littman94}).\\
...
An improvement of the Minimax assumption of optimal opponent behaviour can be achieved by learning opponent models. Those models can either be learned by the $M^{*}$ algorithm described in \cite{opponentModel} or simply by counting how often an agent executed action $a$ in state $s$:
\begin{equation}
...
In distributed control the agents are the controllers and the environment is the controlled process. A domain of application here is the control of traffic light systems (\cite{distributedcontrol}).\\
