...
int sendTokenOnSocket(int sd, const char* token) {
	...
	char *temp = malloc(len + 2); // We need 1 more for separator
	memcpy(temp, token, len);
	temp[len] = SEPARATOR;
	len++;
	temp[len] = 0;
	// We send the message
	int rc = send(sd, temp, len, 0);
	if(rc != len) {
...
int sendActionsOnSocket(int sd, double actions[]) {
	...
	sprintf(buffer, "%s%c%d%c", CMD_STEP, SEPARATOR, numActions, SEPARATOR);
	len = strlen(buffer);
...
int performInit(int sd, int argc, const char *agent_params[]) {
	...
	if(rc >= 0) agent_init(numStates, numActions, argc, agent_params);
	if(rc >= 0) {
	...
	if(rc >= 0) {
		if(terminal == 1) {
			...
			while(terminal == 0 && rc >= 0) {
				...
				if(terminal == 0) pActions = agent_step(pStates, reward);
			}
			if(rc >= 0) {
