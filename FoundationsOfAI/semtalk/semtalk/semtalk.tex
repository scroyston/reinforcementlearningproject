...
\begin{frame}
  ...
  \begin{itemize}
  \item learning by \textcolor{red}{interacting with the environment}
  \item taking actions and receiving \textcolor{red}{rewards}
...
\begin{itemize}
\item just \textcolor{red}{"raw data"} without labeling given
\item no "teacher"
...
\item goal: \textcolor{red}{maximize long-term reward}
\end{itemize}
\end{itemize}
\end{frame}         
...
\end{equation}
\end{block}
The probability distribution of the next state \textcolor{red}{only depends on the previous state} and not on all the states visited before
\end{frame}
\begin{frame}
...
\end{equation}
is called the state-value function for policy $\pi$. $E_{\pi}{}$ is the expected value assuming the agent follows the policy $\pi$ with an arbitrary time step $t$.
\end{definition}
...
\item may be \textcolor{red}{deterministic} or \textcolor{red}{stochastic}
\begin{itemize}
\item \textbf{deterministic policies}: action is clearly specified
\item \textbf{stochastic policies}: actions are taken with a defined probability (necessary in many games)
\end{itemize}
...
A mapping $\pi:S \rightarrow PD(A) \ \forall s \in S$ is called a policy . $PD(A)$ denotes a probability distribution of the action set.
\end{definition}
\end{frame}
\begin{frame}
...
\begin{itemize}
\item do not require full world knowledge ($\rightarrow$ \textcolor{red}{model free})
...
\begin{itemize}
\item Assumption: World is \textcolor{red}{completely known} (possible states, transition probabilities, rewards...)
\item \textcolor{red}{Idea}:
\begin{itemize}
...
\framesubtitle<1>{Initial Situation with initial policy}
\framesubtitle<2>{Situation after first value update}
\framesubtitle<3>{Situation after policy has been adapted}
\framesubtitle<4>{Situation after final value update}
  \begin{tikzpicture}[overlay,scale=2.0,transform shape]
            \node[start] at (1.5,0) (a) {0};
...
\begin{itemize}
\item Markov Decision Processes as framework no longer sufficient in most cases $\rightarrow$ \textcolor{red}{Markov Games} as a new framework
\item Different \textcolor{red}{relations} between agents have to be considered
\begin{itemize}
\item Fully \textcolor{red}{cooperative} (robot swarm exploring the world, soccer players of one team)
\item Fully \textcolor{red}{competitive} (playing a game against other agents)
\item \textcolor{red}{Mixed}
\end{itemize}
\item \textcolor{red}{coordination} between the agents
\item \textcolor{red}{complexity} rises $\rightarrow$ exponential in the number of agents
...
\item \textcolor{red}{information} about the \textcolor{red}{other agents} has to be collected (agent model)
\end{itemize}
\end{frame}
...
\begin{itemize}
\item a set of states $\mathcal{S}$
\item a \textcolor{red}{collection of action sets} $\mathcal{A}_{1},...,\mathcal{A}_{k}$ for \textcolor{red}{each agent}
...
\end{equation}
\item reward functions $R_{i}$ for \textcolor{red}{each agent} $i$:
\begin{equation*}
...
\end{frame}
\begin{frame}
\frametitle{Classification of MARL Tasks}
\begin{itemize}
\item \textbf{Fully cooperative tasks}: All agents have the \textcolor{red}{same goal}\\
Example: Explore the world most efficiently
\item \textbf{Fully competitive tasks}: Agents have an \textcolor{red}{opposite goal} (try to minimize the reward of the other agent)\\
Example: Try to win against the agents of the other team in robot soccer
\item \textbf{Mixed tasks}: Agents \textcolor{red}{cooperate}, but also act in an \textcolor{red}{egoistic} way\\
Example: Two robots cleaning a big and a small floor. The joint task is to clean the whole building, but each robot wants to clean the smaller floor in order to have less work
\begin{itemize}
\item \textbf{agent independent}:
Common structure based on Q-Learning is shared between the agents
\item \textbf{agent tracking}:
Decision is based on a model of the other agents
\item \textbf{agent aware}:
Agent tries to adapt to the other agents
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item \textcolor{red}{Reward functions} of agents are \textcolor{red}{identical} ($\mathcal{R}_{1}=...=\mathcal{R}_{n}$)
\item Naive approach:
\begin{itemize}
\item Use \textcolor{red}{joint Q-Function} (i.e. each agent uses the same function table) and update it with the Q-Learning algorithm
\item Use \textcolor{red}{greedy policy} to maximize the \textcolor{red}{common return}:
\begin{equation*}
h_{i}^{*}(s)= arg \max_{a_{i}}{\max_{a_{1},...,a_{k}}{Q^{*}(s,a_{1},...,a_{k})}}
\end{equation}
\end{itemize}
\item Seems nice, but what happens if there are more than one maximizing action tuples? $\rightarrow$ \textcolor{red}{Coordination Problem}
\item Additional problem: \textcolor{red}{Q tables} may become very \textcolor{red}{large} (all combinations of actions of the agents have to be considered)
\end{itemize}
\end{frame}
\begin{frame}
...
\item Idea: maximize your own benefit under the assumption that your opponent \textcolor{red}{always tries to minimize it}
\item Important algorithm: \textcolor{red}{Minimax-Q}
\item bad performance against agents playing not optimal
\item improvement: learning \textcolor{red}{opponent models}, e.g. by counting, how many times an agent $i$ ovserved an agent $j$ taking an action $a_{j}$ in state $s$:
\begin{equation*}
...
\begin{algorithmic}
\STATE $\forall s \in \mathcal{S},a \in \mathcal{A}, o \in \mathcal{O}: \ Q(s,a,o) \gets 1$
\STATE $\forall s \in \mathcal{S}: \ V(s) \gets 1$
\STATE $\forall s \in \mathcal{S}, a \in \mathcal{A}: \ \pi(s,a) \gets 1/|A|$
...
\begin{algorithmic}
\STATE return a random action with probability $explore$, otherwise:
...
\end{tikzpicture}
\begin{itemize}
\item random ball possession at the beginning
\item "blocker" gets ball possession
\item goal: "walk" into enemy's goal
\item four policies were trained: MM, QR, QQ, MR (Minimax-Q against itself, Q-Learning against random,...)
\item challenger opponent: opponent trained against each of those policies
\end{itemize}
\end{frame}
\begin{frame}
...
\begin{itemize}
\item Minimax-Q is \textcolor{red}{insensitive} against the opponent against which it was trained
\item Minimax assumption \textcolor{red}{not good} against \textcolor{red}{non-optimal} opponents
\item Minimax-Q uses stochastic policies, Q-Learning only deterministic (reason for always losing against challenger)
\end{itemize}
\end{frame}
\begin{frame}
...
\begin{itemize}
\item \textcolor{red}{no restrictions} to the reward function
\item \textcolor{red}{General sum games}
\item strongly influenced by game theoretic \textcolor{red}{equilibrium concepts}
\item distinction between \textcolor{red}{agent-independent}, \textcolor{red}{agent-tracking} and \textcolor{red}{agent-aware} methods, Here: only agent-independent methods
\item Two approaches
\begin{itemize}
\item Apply \textcolor{red}{single agent algorithms} (like Q-Learning) to the multiagent case (problem: nonstationarity, only sometimes converts to an equilibrium, advantage:very simple)
\item find \textcolor{red}{equilibria} for stage games
\end{itemize}
\item Important algorithm: Nash-Q Learning
\item problem: Which equilibrium to choose if there is more than one equilibrium? $\rightarrow$ \textcolor{red}{equilibrium selection problem}
\end{itemize}
\end{frame}
\begin{frame}
